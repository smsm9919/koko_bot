...
# strategy_guard.py — Balanced strategy + strong anti-reverse protections
...
def attach_guard(userbot):
    # Balanced defaults: flexible entry, strict protections
    ...
    # Wrap close_position: show cumulative profit
    ...
    # Wrap place_order: balanced entry + strong protections + verbose logs
    ...
    def _wrap_place(side, qty):
        ...
        # hourly cap
        now = time.time()
        while recent_ts and now - recent_ts[0] > 3600: recent_ts.popleft()
        if len(recent_ts) >= MAX_TRADES_PER_HOUR:
            _no_trade("max trades/hour reached"); return False
